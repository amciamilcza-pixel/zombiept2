import os
from ecg_signals import (load_normal, load_arrhytmia, load_bradycardia, add_real_noise)
...
from plots import (plot_main_noisy_vs_recovered, plot_dft_filter_explanation,
    plot_stress_tests, plot_dft_failure)
...
_, ecg_arrhythmia = load_arrhytmia(duration=DURATION)
_, ecg_bradycardia = load_bradycardia(duration=DURATION)
...
# Returns cleaned heartbeat signal in time domain, DFT spectrum of noisy signal, DFT spectrum after filtering and frequency axis in Hz
for name, clean_signal in signals_clean.items():
    ...
    # Saves noisy and recovered DFT spectrum and the frequency axis
    recovery_info[name] = {
        "X_noisy": X_noisy,
...
# Create figure 1 - noisy vs recovered ECG for each case
plot_main_noisy_vs_recovered(t, signals_noisy, signals_recovered, FS)

# Create figure 2 - DFT spectrum before and after filtering using the normal heartbeat
...
plot_stress_tests(window_data, noise_data)

# Create figure 4 - failure test: runs arrythmia signal and compares one DFT over whole signal vs repeated DFTs over short windows
failure_data = failure_cases(fs=FS)
plot_dft_failure(failure_data)
